...
public class ContactWXActivity extends BaseSlideBackActivity implements
...
//				R.layout.layout_titlebar_default);// 更换自定义标题栏布局
	...
	private void initView() {
		emptyView = findViewById(R.id.wx_empty);
		// 标题栏
		...
		tvTitle.setText(R.string.title_activity_contact_wx);
		...
		pullRefreshListView = (PullToRefreshListView) findViewById(R.id.contact_wx_pull_refresh_listview);
	...
	private void initItemAdapter() {
		...
		adapter.setOnContactClickListener(new OnContactClickListener() {
			...
			public void onEdit(String ID, String name, String content) {
	        	...
	        	ContactWXEdtDialogFragment showView;
	        		showView = ContactWXEdtDialogFragment.newInstance(ID, name, content);
	        	...
	        	showView.show(vThis.getSupportFragmentManager(), "ContactWXEdtDialogFragment");
			}
	...
	public void onClick(View v) {
		...
		case R.id.titlebar_btnRight:
			ContactWXEdtDialogFragment showView;
    		showView = ContactWXEdtDialogFragment.newInstance("", "", "");
		...
		protected String doInBackground(Void... params) {
			...
			try {
				itemList = ShopSetAPI.getInstance()
						.GetContactInfoList(3, PublicData.getCookie(vThis));
