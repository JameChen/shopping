 ...
 * @author ZZB
 */
public class BankAuthActivity extends BaseActivty implements OnClickListener {
    ...
    private TextView mETCardNo,tv_bank_name;
    ...
    private void initParams() {
        ...
        mBankInfo = UserInfoProvider.getBankInfo(mContext, SpManager.getUserId(mContext));
//        if (mAuthInfo == null) {
...
//        }
    ...
    private void initView() {
        ...
        mETCardNo = (TextView) findViewById(R.id.et_card_number);
    ...
    private void updateUI2() {
        String authState = mBankInfo.getStatu();
        mETUserName.setText(mBankInfo.getRealName());
        mETCardNo.setText(mBankInfo.getCardNumber());
        tv_bank_name.setText(mBankInfo.getBank());
        if (Const.IDAuthState.AUTH_PASSED.equals(authState)) {// 验证通过
          ...
          //  updateBottomInfo("亲，必须是本人的证件，证件姓名与提现银行卡姓名必须一致，否则无法提现\n\n审核时间为1-3个工作日");
           ...
           // mETUserName.setText(mAuthInfo.getRealName());
            ...
            updateBottomInfo(Html.fromHtml("驳回原因：<font color='red'>" + mBankInfo.getMessage() + "</font>"));
        } else if (Const.IDAuthState.RECHECK.equals(authState)) {// 重审
            //changeView(false);
            mTVCardState.setText(Const.IDAuthState.RECHECK);
            updateBottomInfo(Html.fromHtml("重审原因：<font color='red'>" + mBankInfo.getMessage() + "</font>"));
        }
    }
