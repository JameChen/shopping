...
import com.nahuo.quicksale.hyphenate.model.EaseCompat;
import com.nahuo.quicksale.hyphenate.model.EaseDingMessageHelper;
import com.nahuo.quicksale.hyphenate.utils.EaseCommonUtils;
import com.nahuo.quicksale.hyphenate.utils.EaseUserUtils;
import com.nahuo.quicksale.hyphenate.widget.EaseAlertDialog;
import com.nahuo.quicksale.hyphenate.widget.EaseAlertDialog.AlertDialogUser;
import com.nahuo.quicksale.hyphenate.widget.EaseChatExtendMenu;
import com.nahuo.quicksale.hyphenate.widget.EaseChatInputMenu;
import com.nahuo.quicksale.hyphenate.widget.EaseChatInputMenu.ChatInputMenuListener;
import com.nahuo.quicksale.hyphenate.widget.EaseChatMessageList;
import com.nahuo.quicksale.hyphenate.widget.EaseVoiceRecorderView;
import com.nahuo.quicksale.hyphenate.widget.EaseVoiceRecorderView.EaseVoiceRecorderCallback;
import com.nahuo.quicksale.hyphenate.widget.chatrow.EaseCustomChatRowProvider;
 ...
 */
public class EaseChatFragment extends com.nahuo.quicksale.hyphenate.ui.EaseBaseFragment implements EMMessageListener {
    ...
    protected ShopItemModel mSendItem;
    protected RxPermissions rxPermissions;
    private boolean FIRST_SAVEIMMESSAGE = true;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(com.nahuo.quicksale.R.layout.pinhuo_ease_fragment_chat, container, false);
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (fragmentArgs != null) {
            if (fragmentArgs.containsKey(EaseConstant.EXTRA_DETAIL_ITEM)) {
                mSendItem = (ShopItemModel) fragmentArgs.get(EaseConstant.EXTRA_DETAIL_ITEM);
            }
        }
...
//        messageList.setAvatarShape(1);
    ...
    protected void setUpView() {
        ...
        if (toChatUsername.equals(String.valueOf(ChatUtl.SYSTEM_HX_ID))) {
            titleBar.setTitle(ChatUtl.SYSTEM_NAME);
            inputMenu.setVisibility(View.GONE);
        } else {
            titleBar.setTitle(SpManager.getECC_USER_NAME(getActivity())+ChatUtl.ECC_KEFU_DEP);
                    ...
                    rxPermissions.request(Manifest.permission.CAMERA).subscribe(new Observer<Boolean>() {
                        ...
                        public void onNext(Boolean aBoolean) {
                            if (aBoolean) {
                                ...
                                ViewHub.showShortToast(getActivity(), getString(R.string.picture_camera));
    ...
    protected void sendMessage(EMMessage message) {
        ...
        String name = SpManager.getUserName(getActivity());// PublicData.mUserInfo.getUserName();
        String userid = String.valueOf(SpManager.getUserId(getActivity()));
        ...
        try {
            ...
            JSONArray tags = new JSONArray(SpManager.getECC_TAG(getActivity()));
            visitorJson.put("tags", tags);
            weichatJson.put("visitor", visitorJson);


            weichatJson.put("queueName", SpManager.getECC_QUEUE_NAME(getActivity()));
            weichatJson.put("agentUsername", SpManager.getECC_AGENT_USER_NAME(getActivity()));
            ...
            if (FIRST_SAVEIMMESSAGE) {
                ...
                if (messageBody != null) {
                    if (messageBody instanceof EMTextMessageBody) {
                        mess = ((EMTextMessageBody) messageBody).getMessage();
                    } else if (messageBody instanceof EMImageMessageBody) {
                        mess = ((EMImageMessageBody) messageBody).getFileName();
                    } else if (messageBody instanceof EMVoiceMessageBody) {
                        mess = ((EMVideoMessageBody) messageBody).getFileName();
                    } else if (messageBody instanceof EMNormalFileMessageBody) {
                        mess = ((EMNormalFileMessageBody) messageBody).getFileName();
                    } else if (messageBody instanceof EMLocationMessageBody) {
                        mess = ((EMLocationMessageBody) messageBody).getAddress();
                    }
                ...
                saveImMessage(jsonObject.toString());
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    protected void saveImMessage(String json) {

        addSubscribe(HttpManager.getInstance().getPinHuoNoCacheApi(TAG).saveImMessage(json)
                .compose(RxUtil.<PinHuoResponse<Object>>rxSchedulerHelper())
                .compose(RxUtil.<Object>handleResult())
                .subscribeWith(new CommonSubscriber<Object>(getActivity()) {
                    @Override
