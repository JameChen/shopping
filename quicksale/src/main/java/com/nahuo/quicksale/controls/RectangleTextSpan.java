...
/**
 * Created by jame on 2018/8/20.
 */

public class RectangleTextSpan extends ReplacementSpan {
    ...
    public RectangleTextSpan(Context context, int bgColorResId, int textColorResId, String text) {
        if (TextUtils.isEmpty(text)) {
            return;
        }
        //初始化默认数值
        initDefaultValue(context, bgColorResId,textColorResId, text);
        //计算背景的宽度
     ...
     * @param context
     */
    private void initDefaultValue(Context context, int bgColorResId, int textColorResId,String text) {
    ...
    private float caculateBgWidth(String text) {
        if (text.length() > 1) {
            ...
            float padding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 8, mContext.getResources().getDisplayMetrics());
